...
include_once(substr(dirname( __FILE__ ), 0,-strlen($current_dir) ) . '/202-config/functions-tracking202.php');
...
if (isset($_POST['database_management'])) {
    ...
    if (isset($clickid)) {
        foreach ($tables as $table) {
                ...
                $click_sql = "DELETE FROM $table WHERE click_id < ".$clickid;
	...
	//set the auto increment to the max value in the 202_clicks table +1 this prevents it from being reset to zero if the server gets restarted
	$click_sql ="select max(click_id) as click_id from 202_clicks";
	$result = _mysqli_query($click_sql);
	$row = $result->fetch_assoc();
	
	$click_id=$db->real_escape_string($row['click_id']);
	$click_id+=1;
	
	$click_sql="ALTER TABLE 202_clicks_counter AUTO_INCREMENT = ".$click_id;
	_mysqli_query($click_sql);
			...
			<div class="form-group">
				...
				<div class="input-group input-group-sm">
					<input type="number" class="form-control input-sm"
						...
						every click that occured before the specified number of days. <strong>(Enter 0 or leave blaank if you don't want old data autormatically deleted)</strong></span>
				</div>
			</div>
			<div class="form-group">
				<div class="col-sm-6 col-sm-offset-6">
