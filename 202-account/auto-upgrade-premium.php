...
include_once(substr(dirname( __FILE__ ), 0,-strlen($current_dir) ) . '/202-config/functions-upgrade.php');
include_once(substr(dirname( __FILE__ ), 0,-strlen($current_dir) ) . '/202-config/functions-rss.php');
...
$json = @getData('http://my.tracking202.com/api/v2/premium-p202/version');
...
if ($_POST['start_upgrade'] == '1' and getenv("P202_MANAGED_UPDATES") == false) {
		...
		$GetUpdate = @getData('http://my.tracking202.com/api/v2/premium-p202/download/'.$user_row['install_hash'].'/'.$user_row['p202_customer_api_key']);
...
<?php } else if ($update_needed == true) { info_top();
