...
function setClickIdCookieForLp($click_id, $lp_public_id)
{   
    $expire = time() + 2592000;
    setcookie('tracking202rlp_' . $lp_public_id, $click_id, $expire, '/', $_SERVER['SERVER_NAME']);
}
        ...
        if (!$getFromCache) {
            ...
            foreach ($ranges as $key => $value) {
                if (PLATFORMS::check_ip_range($ip, $value)) {
                    if ($memcacheWorking) {
                        setCache(md5("ip-bot" . $ip . systemHash()), true);
                    }
        ...
        if ($memcacheWorking) {
            ...
            if ($getID) {
                ...
                if ($ip_row) {
                    ...
                    $ip_id = $ip_row['ip_id'];
                    // add to memcached
                    $setID = setCache(md5("ip-id" . $ip_address . systemHash()), $ip_id, $time);
                    return $ip_id;
                } else {
                    // else if this doesn't exist, insert the new iprow, and return the_id for this new row we found
...
function memcache_set_user_key($sql)
{
    if (AUTH::logged_in() == true) {
        ...
        $setCache = setCache(md5($user_id, $queries . systemHash()), false);
    }
}

function memcache_mysql_fetch_assoc($db, $sql, $allowCaching = 1, $minutes = 1)
{
    global $memcacheWorking, $memcache;
