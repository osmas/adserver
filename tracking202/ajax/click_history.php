...
LEFT JOIN 202_rotators AS 2r ON (2c.rotator_id = 2r.id)		    
...
//now display all the clicks
	while ($click_row = $click_result->fetch_array(MYSQLI_ASSOC)) {   
		...
		if ($click_row['aff_campaign_name'] != null && $click_row['rotator_id'] ){
		    if($click_row['landing_page_nickname'] != null){
		        $html['aff_campaign_name'] = htmlentities("Redirector: ".$click_row['rotator_name']." > LP: ".$click_row['landing_page_nickname']." > Offer: ".$click_row['aff_campaign_name'], ENT_QUOTES, 'UTF-8');
		    }
		        else{
		            $html['aff_campaign_name'] = htmlentities("Redirector: ".$click_row['rotator_name']." > Offer: ".$click_row['aff_campaign_name'], ENT_QUOTES, 'UTF-8');
		        }
		 	
		} else if($click_row['aff_campaign_name'] != null) {
		    $html['aff_campaign_name'] = htmlentities($click_row['aff_campaign_name'], ENT_QUOTES, 'UTF-8');
		}
		    else {
			$html['aff_campaign_name'] = htmlentities("Redirector:".$click_row['rotator_name']." > LP: ".$click_row['landing_page_nickname'], ENT_QUOTES, 'UTF-8');
		}
		...
		<?php } ?>
			...
			<tr <?php if (($diff <= 5) and ($new == true)) {echo 'class="new-click" style="display:none;"';}?>>
				...
				<td class="referer" >
					...
					<?php if (($html['outbound'] != '') and ($click_row['click_out'] == 1) or ($click_row['rotator_name'])) { printf('<a href="%s" target="_new"><img src="%s202-img/icons/16x16/control_play_blue.png" alt="Outbound" title="Outbound: %s"/></a>',$html['outbound'],get_absolute_url(),$html['outbound']); } ?>
					...
					<?php if (($html['redirect'] != '') and ($click_row['click_out'] == 1) or ($click_row['rotator_name'])) { printf('<a href="%s" target="_new"><img src="%s202-img/icons/16x16/control_fastforward_blue.png" alt="Redirection" title="Redirect: %s"/></a>',$html['redirect'],get_absolute_url(),$html['redirect']); } ?>
				</td>
