...
if (isset($_GET['dni']) && isset($_GET['all_offers'])) {
	...
	$sql = "SELECT dni.networkId, dni.apiKey, dni.affiliateId, dni.name, 2u.install_hash, 2up.user_account_currency FROM 202_dni_networks AS dni LEFT JOIN 202_users AS 2u USING (user_id) LEFT JOIN 202_users_pref AS 2up USING (user_id) WHERE dni.user_id = '".$mysql['user_id']."' AND dni.id = '".$mysql['dni_id']."'";
	...
	if ($results->num_rows > 0) {
		...
		echo getDniOffers($dni['install_hash'], $dni['networkId'], $dni['apiKey'], $dni['affiliateId'], $dni['user_account_currency'], $_GET['offset'], $_GET['limit'], $sort_by, $filter_by);
	}
}
...
if (isset($_GET['dni']) && isset($_GET['offer_id']) && isset($_GET['setup_offer'])) {
	$sql = "SELECT dni.networkId, dni.apiKey, dni.affiliateId, 2u.install_hash, 2up.user_account_currency FROM 202_dni_networks AS dni LEFT JOIN 202_users AS 2u USING (user_id) LEFT JOIN 202_users_pref AS 2up USING (user_id) WHERE dni.user_id = '".$mysql['user_id']."' AND dni.id = '".$mysql['dni_id']."'";
	$results = $db->query($sql);
	if ($results->num_rows > 0) {
		$dni = $results->fetch_assoc();
		...
		$offerData = setupDniOffer($dni['install_hash'], $dni['networkId'], $dni['apiKey'], $dni['affiliateId'], $dni['user_account_currency'], $_GET['offer_id'], $_GET['ddlci']);
