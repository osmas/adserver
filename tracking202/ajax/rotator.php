...
if (isset($_POST['get_rotators']) && isset($_POST['rotator_id']) && $_POST['get_rotators'] == true) { ?>
	...
	<select class="form-control input-sm" name="tracker_rotator" id="the_tracker_rotator">
	    <option value=""> -- </option>
		...
		foreach ($_POST['data'] as $rule) {
			...
			if ($rule_result) {
				foreach ($rule['criteria'] as $criteria) {
					...
					if (strcasecmp("ALL", $value) == 0) {
					    $value = 'ALL'; //save ALL as uppercase to elimitate errors cause by how users typed the value
					}
...
if (isset($_POST['rule_id']) && $_POST['rule_details'] == true) {
	$id = $db->real_escape_string($_POST['rule_id']);
	$rule_sql = "SELECT * FROM 
				 202_rotator_rules AS ru 
				 LEFT JOIN 202_rotator_rules_criteria AS cr ON ru.id = cr.rule_id
				 WHERE ru.id = '".$id."'";
	$rule_result = $db->query($rule_sql);

	$redirect_sql = "SELECT rr.redirect_url, rr.redirect_campaign, rr.redirect_lp, rr.weight, 202_aff_campaigns.aff_campaign_name, 202_landing_pages.landing_page_nickname FROM 202_rotator_rules_redirects AS rr 
	LEFT JOIN 202_aff_campaigns ON (rr.redirect_campaign = 202_aff_campaigns.aff_campaign_id)
	LEFT JOIN 202_landing_pages ON (rr.redirect_lp = 202_landing_pages.landing_page_id)
	WHERE rr.rule_id = '".$id."'";
	$redirect_result = $db->query($redirect_sql);
	?>
	...
	<div class="row">
		...
		<span class="infotext">Here you can see criteria for rule.</span>
			<table class="table table-bordered" id="stats-table" style="margin-top: 10px;">
				...
				<tbody>
				<?php while ($row = $rule_result->fetch_assoc()) {
			...
			<div class="col-xs-12">
				<div class="row">
				<?php while ($row = $redirect_result->fetch_assoc()) { ?> 
					<div class="col-xs-12" style="margin-bottom: 15px;">
						<div class="row">
							<div class="col-xs-3">
					    		<div class="small">Redirects to (<?php if ($row['weight'] == null || $row['weight'] == '0') echo '100'; else echo $row['weight']; ?>%): </div>
					    	</div>
						    <div class="col-sm-9">
						    <?php if($row['aff_campaign_name'] != null) { ?>
						    	<div class="small">
						    		<span class="label label-info"><i><?php echo $row['aff_campaign_name'];?></i></span> campaign
						    	</div>
						    <?php } else if ($row['redirect_url'] != null) { ?>
						      	<code style="font-size: 12px;"><?php echo $row['redirect_url'];?></code>
							<?php } else if ($row['landing_page_nickname'] != null) { ?>
								<div class="small">
						    		<span class="label label-info"><i><?php echo $row['landing_page_nickname'];?></i></span> landing page
						    	</div>
							<?php } ?>
						    </div>
					    </div>
					...
					<div class="criteria" id="criteria" data-criteria-id="none">
						<div class="form-group">
							...
							<select class="form-control input-sm" name="rule_type" style="margin: 0px 5px;">
								...
								<option value="t202kw">t202kw</option>
													...
													<div class="criteria" id="criteria" data-criteria-id="<?php echo $criteria_row['id'];?>">
														<div class="form-group">
															...
															<select class="form-control input-sm" name="rule_type" style="margin: 0px 5px;">
																...
																<option value="iprange" <?php if($criteria_row['type'] == 'iprange') echo "selected";?>>IP Range</option>
																...
																<option value="visitor" <?php if($criteria_row['type'] == 'visitor') echo "selected";?>>Visitor</option>
																<option value="c1" <?php if($criteria_row['type'] == 'c1') echo "selected";?>>C1</option>
																<option value="c2" <?php if($criteria_row['type'] == 'c2') echo "selected";?>>C2</option>
																<option value="c3" <?php if($criteria_row['type'] == 'c3') echo "selected";?>>C3</option>
																<option value="c4" <?php if($criteria_row['type'] == 'c4') echo "selected";?>>C4</option>															
																<option value="t202kw" <?php if($criteria_row['type'] == 't202kw') echo "selected";?>>t202kw</option>
																<option value="referer" <?php if($criteria_row['type'] == 'referer') echo "selected";?>>Referer</option>
																<option value="utm_source" <?php if($criteria_row['type'] == 'utm_source') echo "selected";?>>utm_source</option>
																<option value="utm_medium" <?php if($criteria_row['type'] == 'utm_medium') echo "selected";?>>utm_medium</option>
																<option value="utm_campaign" <?php if($criteria_row['type'] == 'utm_campaign') echo "selected";?>>utm_campaign</option>
																<option value="utm_term" <?php if($criteria_row['type'] == 'utm_term') echo "selected";?>>utm_term</option>
																<option value="utm_content" <?php if($criteria_row['type'] == 'utm_content') echo "selected";?>>utm_content</option>
															</select>
										  				</div>
