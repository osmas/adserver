...
if ($_GET['default_ppc_account_id']) {

	$mysql['user_id'] = $db->real_escape_string($_SESSION['user_id']);
	$mysql['ppc_account_id'] = $db->real_escape_string($_GET['default_ppc_account_id']);

	$default_network_reset="UPDATE 202_ppc_accounts SET ppc_account_default=''";
	$default_network_set="UPDATE 202_ppc_accounts SET ppc_account_default= '1' WHERE `ppc_account_id`='".$mysql['ppc_account_id']."'";

	$default_ppc_network_result = _mysqli_query($default_network_reset) ;
	$default_ppc_network_result = _mysqli_query($default_network_set) ;
}
		...
		<div class="row">
			...
			<div class="col-xs-7">
				<div class="<?php if($error) echo "error"; else echo "success";?> pull-right" style="margin-top: 20px;">
					<small>
						<?php if ($error) { ?> 
							<span class="fui-alert"></span> There were errors with your submission. <?php echo $error['token']; ?> <?php echo $error['invalid_pixel_url']; ?>
						<?php } ?>
	...
	<div class="col-xs-4 col-xs-offset-1">
		<div class="panel panel-default">
			...
			while ($ppc_network_row = $ppc_network_result->fetch_array(MYSQLI_ASSOC)) {
				...
				$html['ppc_network_name'] = htmlentities($ppc_network_row['ppc_network_name'], ENT_QUOTES, 'UTF-8');
				$html['ppc_network_default'] = htmlentities($ppc_network_row['ppc_network_default'], ENT_QUOTES, 'UTF-8');
				$url['ppc_network_id'] = urlencode($ppc_network_row['ppc_network_id']);

				if($html['ppc_network_default']==1){
					$default_network_code = ' - <span clas="#tracking202_sponsors"><strong>Current Default</strong></span>';	
				}
				else{
					$default_network_code = ' - <a href="?default_ppc_network_id='.$url['ppc_network_id'].'">Make Default</a>';	
				}
				...
				while ($ppc_account_row = $ppc_account_result->fetch_array(MYSQLI_ASSOC)) {
					...
					$html['ppc_account_name'] = htmlentities($ppc_account_row['ppc_account_name'], ENT_QUOTES, 'UTF-8');
					$html['ppc_account_default'] = htmlentities($ppc_account_row['ppc_account_default'], ENT_QUOTES, 'UTF-8');
					$url['ppc_account_id'] = urlencode($ppc_account_row['ppc_account_id']);

					if($html['ppc_account_default']==1){
					$default_account_code = ' - <span clas="#tracking202_sponsors"><strong>Current Default</strong></span>';	
				}
				else{
					$default_account_code = ' - <a href="?default_ppc_account_id='.$url['ppc_account_id'].'">Make Default</a>';	
				}
					
					if ($userObj->hasPermission("remove_traffic_source_account")) {
						printf('<li>%s - <a href="?edit_ppc_account_id=%s">edit</a> - <a href="?delete_ppc_account_id=%s&delete_ppc_account_name=%s" onclick="return confirmSubmit(\'Are You Sure You Want To Delete This Account?\');">remove</a>%s</li>', $html['ppc_account_name'],$url['ppc_account_id'],$url['ppc_account_id'],$html['ppc_account_name'],$default_account_code);
					} else {
