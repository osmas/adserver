...
if (isset($_COOKIE['tracking202rlp_' . $landing_page_id_public])) {
    $click_id = substr($_COOKIE['tracking202rlp_' . $landing_page_id_public], 1, - 1);
    $click_id_public = $_COOKIE['tracking202rlp_' . $landing_page_id_public];
} else {
...
(($tracker_row['click_cloaking'] == - 1) and ($tracker_row['aff_campaign_cloaking'] == 1)) or ((! isset($tracker_row['click_cloaking'])) and ($tracker_row['aff_campaign_cloaking'] == 1))) // if no tracker but but by default campaign has cloaking on
{
    $cloaking_on = true;
