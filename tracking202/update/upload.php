...
include_once(substr(dirname( __FILE__ ), 0,-strlen($current_dir) ) . '/202-config/class-dataengine-slim.php');
include_once(substr(dirname( __FILE__ ), 0,-strlen($current_dir) ) . '/202-config/convlogs.php');
...
switch ($_GET['case']) { 
			...
			if (is_numeric($click_id)) { 
				...
				$click_sql = "
					...
					FROM
						202_clicks AS 2c
					WHERE
						2c.click_id ='". $mysql['click_id']."' AND 2c.user_id='".$mysql['user_id']."'";
				...
				addConversionLog($mysql['click_id'], '', $mysql['aff_campaign_id'], $mysql['click_payout'], $mysql['user_id'], $mysql['click_time'], $_SERVER['HTTP_X_FORWARDED_FOR'], $_SERVER['HTTP_USER_AGENT'],'', 0);
	...
	default:
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
			...
			if(!in_array(end(explode('.', $_FILES['csv']['name'])), array('csv'))) {
				template_top('Upload Revenue Reports',NULL,NULL,NULL); 
				about_revenue_upload();
				echo '<div class="error"><small><span class="fui-alert"></span>Uploaded file should be .csv file.</div>';
				template_bottom();
				die();
			}
